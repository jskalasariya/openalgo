...
class LoggingManager:
    ...
    def get_leg_logger(self, leg_num: int, leg_name: str) -> logging.Logger:
        ...
        if leg_num in self.leg_loggers:
            return self.leg_loggers[leg_num]

        # Create leg log filename
        safe_name = leg_name.replace(' ', '_').lower()
        log_filename = self.date_logs_dir / f"leg{leg_num}_{safe_name}.log"

        # Create logger with unique name
        logger_name = f'NiftySehwag.Leg{leg_num}'
    ...
    def log_to_both(self, leg_num: int, level: str, message: str):
        ...
        if leg_num in self.leg_loggers:
            leg_logger = self.leg_loggers[leg_num]
    ...
    def close_leg_logger(self, leg_num: int):
        ...
        """
        if leg_num in self.leg_loggers:
            logger = self.leg_loggers[leg_num]
            logger.info(f"=== Leg {leg_num} Log Ended ===")
