"""
Nifty Sehwag Persistence Manager
=================================
Handles database persistence for the Nifty Sehwag strategy.
...
class NiftySehwagPersistence:
    """Manages database persistence for Nifty Sehwag strategy"""
    ...
    def __init__(self, expiry_date: str):
        ...
        session = create_session(
            ...
            notes=f"Nifty Sehwag Strategy Session"
        )
    ...
    def update_position(self, leg_num: int, current_price: float, current_sl: float,
        ...
        if position_id:
            try:
                from database.nifty_sehwag_db import db_session, NiftySehwagPosition
                position = db_session.query(NiftySehwagPosition).filter_by(id=position_id).first()
                if position:
    ...
    def detect_crashed_sessions() -> List[Dict]:
        ...
        try:
            ...
            crashed_sessions = db_session.query(NiftySehwagSession).filter(
                NiftySehwagSession.status == 'RUNNING'
            ).all()
            ...
            for session in crashed_sessions:
                ...
                active_positions = db_session.query(NiftySehwagPosition).filter(
                    NiftySehwagPosition.session_id == session.id,
                    NiftySehwagPosition.status == 'ACTIVE'
                ).all()
    ...
    def mark_sessions_as_crashed(session_ids: List[str], crash_reason: str = "Process terminated unexpectedly"):
        ...
        try:
            for session_id in session_ids:
                session = db_session.query(NiftySehwagSession).filter(
                    NiftySehwagSession.session_id == session_id
                ).first()
    ...
    def mark_positions_as_recovered(position_ids: List[int]):
        ...
        try:
            for pos_id in position_ids:
                position = db_session.query(NiftySehwagPosition).filter(
                    NiftySehwagPosition.id == pos_id
                ).first()
